  ...
  const handleTemplateChange = async (
    ...
    try {
      const response = await fetch(
        `/api/preview?path=${encodeURIComponent(templatePath)}`,
      );
      const data = await response.json();
      ...
      const htmlWithoutStyles = data.html.replace(
        /<style[^>]*>[\s\S]*?<\/style>/gi,
        "",
      );
      ...
      // 生成表单字段配置
      const fields: FormField[] = Array.from(placeholders).map(
        (placeholder) => ({
          key: placeholder,
          label: placeholder
            // .replace(/_en$/, '')
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const apiResponse = await fetch(
        `/api/feishu?student_id=${encodeURIComponent(searchId)}`,
      );
      ...
      if (!foundItem) {
        console.log(
          ...
          responseData.data.items.map(
            (item: { fields: FeishuFields }) => item.fields?.["学生学号"],
          ),
        );
        throw new Error(
          `未找到学号为 "${searchId}" 的记录，请确认学号是否正确`,
        );
      }
